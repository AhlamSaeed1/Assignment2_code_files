...
function better_input(v, e)
{
    ...
    for (var c1 = 0; c1 < this.nv; c1++)
    {
        this.vert[c1] = new Vertex(v[c1]);
    }
    ...
    for (var c2 = 0; c2 < this.ne; c2++)
    {
        this.add_edge2(e[c2].u, e[c2].v, e[c2].w);
    }
...
{ //print the frist line in the output
...
{
    var sv, tv;
    // fetch edge vertices using their id, where sv: source vertex, tv: target vertex
    sv = this.vert[u_i];
    tv = this.vert[v_i];


    // insert (sv,tv), i.e., insert tv (by id) in adjacency list of sv

    sv.adjacent.insert(v_i);


    // insert (tv,sv) if undirected graph (repeat above but reverse vertex order)

    if (!this.digraph)
    {
        tv.adjacent.insert(u_i);
    }
...
function adjacentById()
{
    ...
    for (var c3 = 0; c3 < traversal.length; c3++)
    {
        traversal_array[c3] = traversal[c3].target_v;
    }
...
function topoSearch(type)
{
    ...
    {
        if (!this.vert[c5].visit)
        {
            type == 'DFS' ? this.dfs(c5) : this.bfs(c5);
